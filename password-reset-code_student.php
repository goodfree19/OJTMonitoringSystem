...
$mail_template="<h2> Hi $get_name </h2>
    ...
    <a href='http://localhost/OJTMonitoringSystem/create-new-password_student.php?token=$token&email=$get_email'>Click me</a>";
...
//student rest pasword
if(isset($_POST['password_reset_link_student'])){

    $email1=mysqli_real_escape_string($con,$_POST['email1']);
    $token = md5(rand());
    
    $check_email = "SELECT email FROM users WHERE email='$email1' LIMIT 1 ";
    ...
    //student change password code
    if(isset($_POST['change_password_student']))
    {
            ...
            if(!empty($email) && !empty($new_password) && !empty($confirm_password))
            {
                ...
                if(mysqli_num_rows($check_token_run)>0)
                {
                    if($new_password == $confirm_password)
                    {
                        ...
                        header("Location: create-new-password_student.php?token=$token&email=$email");
                        exit(0);
                        }
                    }
                    else
        ...
        else
        {
            ...
            header("Location: create-new-password_student.php");
            exit(0);
        }
    }
?>